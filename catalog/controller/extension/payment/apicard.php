...
class ControllerExtensionPaymentApicard extends Controller {
	...
	public function index(){
		//$this->language->load('extension/payment/apicard');
        ...
        $curl_data = array(            
                ...
                "success_url" => "https://nord-gaming.ru/index.php?route=extension/payment/apicard/response&order_id=".$order_id."",
                "notice_url" => "https://nord-gaming.ru/index.php?route=extension/payment/apicard/callback&order_id=".$order_id."",                
                "api_key" => $this->config->get('payment_apicard_key'),
                "use_card_payment" => "1"
            );
		...
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/extension/payment/apicard')) {
			return $this->load->view($this->config->get('config_template') . '/template/extension/payment/apicard', $data);
		} else {
			return $this->load->view('/extension/payment/apicard', $data);
		}
	}
    ...
    public function pay(){
        ...
        $this->response->redirect($this->url->link('account/epay', '&order_id='.$order_id , true));
	...
	public function callback(){
        ...
        if($this->request->post['status']=="successful_payment"){
            ...
            $this->model_checkout_order->addOrderHistory($order_id, $this->config->get('payment_apicard_order_status_id'), $comment, $notify = false, $override = false);
        }
	}
}
